...
export default function page() {
   ...
   */
  const fetchMembers = useCallback(async (url: string) => {
    if (!token) { return; }
  ...
  // 페이지네이션 핸들러
  const handleFirstPage = () => { if (links?.first) fetchMembers(links.first.href); };
  const handleLastPage = () => { if (links?.last) fetchMembers(links.last.href); };
  ...
  return (
    <div className="w-full bg-gray-50 min-h-screen p-8 pt-15 lg:px-25 md:px-15 font-sans flex flex-col justify-start items-center">
      ...
      <div className='w-full'>
        ...
        >
          <span className='col-span-3'> 아이디 </span>
          <span className='col-span-3'> 이름 </span>
          <span className='col-span-2'> 연락처 </span>
          <span className='col-span-2'> 탈퇴여부 </span>
          <span className='col-span-2'> 가입날짜 </span>
        </div>
        ...
        <div className='flex flex-col'>
          {members.map(mem => (
            <React.Fragment key={mem.username}>
              ...
              >
                <span className='col-span-3 font-semibold mr-5 truncate'> {mem.username} </span>
                <span className='col-span-3 mr-5 truncate'> {mem.nickname} </span>
                <span className='col-span-2'> {mem.phoneNumber !== '' ? mem.phoneNumber : <span className='text-gray-500'> 정보없음 </span>}</span>
                <div className='col-span-2 flex justify-start items-center'>
                  <span className={`rounded-full py-1 px-4 font-medium ${mem.enabled ? 'bg-[#D9F7E5] text-[#4FA969]' : 'bg-[#FFE8EC] text-[#CE1C4C]'}`}>
                    {mem.enabled ? '사용 중' : '탈퇴'}
                  </span>
                </div>
                <span className='col-span-2'> {mem.createAt.slice(0, 10)} </span>
              </div>
